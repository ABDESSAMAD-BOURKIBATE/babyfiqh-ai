...
import {
    prophets, companions, sahabiyat, tabiin, atbaTabiin, scholars, goldenAges,
...
export const LibraryModal: React.FC<LibraryModalProps> = ({ onClose, onSelect, currentLang }) => {
    const [activeTab, setActiveTab] = useState<'prophets' | 'companions' | 'sahabiyat' | 'tabiin' | 'atba_tabiin' | 'scholars' | 'golden_ages' | 'fiqh'>('prophets');
    ...
    const handleSelect = (item: LibraryItem) => {
        ...
        if (childId) {
            logChildActivity(childId, {
                type: 'story',
                title: item.names[currentLang],
                details: `Opened library item: ${item.names['en'] || item.names[currentLang]}`,
                topic: item.type
            });
        }
        onSelect(item);
    ...
    const getItems = () => {
        ...
        if (activeTab === 'golden_ages') return goldenAges;
    ...
    const getTabStyles = (tab: 'prophets' | 'companions' | 'sahabiyat' | 'tabiin' | 'atba_tabiin' | 'scholars' | 'golden_ages' | 'fiqh') => {
        ...
        if (tab === 'golden_ages') return {
            activeText: 'text-yellow-400',
            activeBg: 'bg-white/5',
            bar: 'bg-yellow-400',
            hover: 'hover:text-yellow-300',
            card: 'bg-yellow-900/20 hover:bg-yellow-900/40 hover:border-yellow-500/50',
    ...
    const getTypeName = (item: LibraryItem) => {
        switch (item.type) {
            ...
            case 'golden_age': return t.goldenAges;
        ...
        >
            ...
            >
                ...
                <div className="flex items-center border-b border-white/10 overflow-x-auto custom-scrollbar shrink-0 bg-black/20">
                    ...
                    <button
                        onClick={() => handleTabChange('golden_ages')}
                        ...
                        ${activeTab === 'golden_ages' ? 'text-yellow-400 bg-white/5' : 'text-white/50 hover:bg-white/5 hover:text-white'}`}
                    >
                        <CrownIcon className="w-5 h-5" />
                        {t.goldenAges}
                        {activeTab === 'golden_ages' && <div className="absolute bottom-0 left-0 right-0 h-0.5 bg-yellow-400 shadow-[0_0_10px_rgba(250,204,21,0.5)]"></div>}
                    </button>
                    <button
                    ...
                    {activeTab === 'fiqh' ? (
                        ...
                        <>
                            {filteredItems.length > 0 ? (
                                <div className={`grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 animate-fade-in ${activeTab === 'golden_ages' ? 'items-start' : ''}`}>
                                    {filteredItems.map((item) => (
                                        <div
                                            key={item.id}
                                            className={`p-4 rounded-xl border border-white/10 text-right transition-all hover:shadow-lg group flex flex-col relative
                                            ${currentStyle.card}`}
                                        >
                                            <button
                                                onClick={() => handleSelect(item)}
                                            ...
                                            >
                                                <div className={`w-12 h-12 rounded-full flex items-center justify-center text-xl font-bold flex-shrink-0 transition-transform group-hover:rotate-12 ${currentStyle.iconBg}`}>
                                                    {activeTab === 'golden_ages' ? <CrownIcon className="w-6 h-6" /> : item.names[currentLang].charAt(0)}
                                                </div>
                                            ...
                                            {/* Golden Ages Figures Section */}
                                            {item.type === 'golden_age' && item.figures && (
                                                <div className="mt-4 pt-3 border-t border-white/10 flex flex-col gap-3">
                                                    {item.figures.men.length > 0 && (
                                                        <div>
                                                            <div className="flex items-center gap-2 mb-1.5 text-blue-300/80 text-xs font-bold uppercase tracking-wider">
                                                                <UserIcon className="w-3 h-3" />
                                                                {t.men}
                                                            </div>
                                                            <ul className="space-y-1 pr-2 border-r-2 border-blue-500/20">
                                                                {item.figures.men.map((fig, i) => (
                                                                    <li key={i}>
                                                                        <button
                                                                            onClick={(e) => {
                                                                                e.stopPropagation();
                                                                                handleSelect({
                                                                                    id: `ga_fig_m_${i}_${item.id}`,
                                                                                    names: fig,
                                                                                    type: 'scholar'
                                                                                });
                                                                            }}
                                                                            className="text-xs text-white/70 hover:text-blue-300 transition-colors text-left w-full flex items-start gap-1"
                                                                        >
                                                                            <span className="opacity-50">•</span>
                                                                            <span>{fig[currentLang] || fig['en']}</span>
                                                                        </button>
                                                                    </li>
                                                                ))}
                                                            </ul>
                                                    ...
                                                    {item.figures.women.length > 0 && (
                                                        <div>
                                                            <div className="flex items-center gap-2 mb-1.5 text-pink-300/80 text-xs font-bold uppercase tracking-wider">
                                                                <UserIcon className="w-3 h-3" />
                                                                {t.women}
                                                            </div>
                                                            <ul className="space-y-1 pr-2 border-r-2 border-pink-500/20">
                                                                {item.figures.women.map((fig, i) => (
                                                                    <li key={i}>
                                                                        <button
                                                                            onClick={(e) => {
                                                                                e.stopPropagation();
                                                                                handleSelect({
                                                                                    id: `ga_fig_f_${i}_${item.id}`,
                                                                                    names: fig,
                                                                                    type: 'scholar'
                                                                                });
                                                                            }}
                                                                            className="text-xs text-white/70 hover:text-pink-300 transition-colors text-left w-full flex items-start gap-1"
                                                                        >
                                                                            <span className="opacity-50">•</span>
                                                                            <span>{fig[currentLang] || fig['en']}</span>
                                                                        </button>
